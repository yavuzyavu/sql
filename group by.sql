-- GROUP BY -- 
/*
GROUP BY cumlecigi bir SELECT İFADESİNDE CSATIRLARI SUTANLARI DGERLERİNE GORE ozet olarak guruplamak icin
kullanilır
...
--kisi isimine gore satilan toplam meyve miktarini gosteren sorguyu yaziniz
...
select urun_adi, min(urun_miktari) as minumum_urun_mitari, max(urun_miktari) as maksimum_urun_miktari
from manav where urun_adi is not null group by urun_adi order by maksimum_urun_miktari
...
select  * from personel
 --personelin calıstıgı ulkeleri listeleyiniz
 ...
 -- maas ortalamasini bayanlar baylar olarak sorgulayiniz
 SELECT cinsiyet , round(avg(maas)) as ortalama_maas 
 from personel
 group by cinsiyet
 
 --Personeli, ulkelere gore ve sehirlere gore gruplayarak sorgulayin
 select ulke, sehir from personel 
 group by ulke,sehir
 
 --Personeli, ulkelere gore ve sehirlere gore calisan sayisini sorgulayin
 select ulke,sehir,count(sehir) as calisan_sayisi from personel 
 group by ulke,sehir
 
 --Her ulke icin bay ve byan calisan sayisini ve yas ortalamasinni sorgula
 SELECT ulke,cisiyet,count(*) as calisan_sayisi, round(avg(yas)) as yas ortalamasi from personel
 ...
 select ulke,cinsiyet, avg(yas),count(*)
 from personel
 where maas>30000
 group by ulke,cinsiyet
